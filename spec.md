...
2.Сервер виконує SQL-запит до локального файлу SQLite.
3.База повертає масив об'єктів турів, сервер вставляє їх у шаблон EJS та віддає готову сторінку у браузер.
4.При кліку на конкретний тур (GET /tours/:id) додатково підтягуються точки для карти (з поля map_coordinates) та відгуки з таблиці reviews.

 Сценарій Б: Додавання туру адміністратором
1.Адмін заповнює форму та надсилає дані (POST /admin/tours).
2.Сервер зчитує дані і виконує SQL-запит до SQLite: INSERT INTO tours (...) VALUES (...).
3.Після успішного запису у файл бази даних сервер робить перенаправлення (redirect) адміна на оновлену сторінку зі списком турів.
...
2.Сервер приймає дані, перевіряє їх і виконує запит до SQLite: INSERT INTO reviews (tour_id, user_id, rating, comment) VALUES (...).
3.Після успішного збереження сервер робить перенаправлення (redirect) користувача назад на сторінку цього ж туру, де вже відображається новий відгук.